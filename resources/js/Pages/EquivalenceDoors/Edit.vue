...
const props = defineProps({
  equivalenceDoors: Object,
  sucursales: Array,
  clientes: Array,
});

const initialSucursales = (props.equivalenceDoors?.sucursal || '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);

const form = useForm({
  client: props.equivalenceDoors?.client ?? '',
  sucursal: props.equivalenceDoors?.sucursal ?? '',
  sucursal_objetivo_ba: props.equivalenceDoors?.sucursal_objetivo_ba ?? '',
});

const selectedSucursales = ref([...initialSucursales]);
...
function submit() {
  ...
  form.put(route('doors.update', props.equivalenceDoors.id), {
    preserveScroll: true,
  });
}
</script>
...
<template>
  <Head title="Editar puerta" />

  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar puerta</h2>
    </template>
